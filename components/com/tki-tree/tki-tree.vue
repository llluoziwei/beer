...
<script>
	export default {
		...
		props: {
			...
			idKey: {
				type: String,
				default: 'bad_reason_id'
			},
			rangeKey: {
				type: String,
				default:'bad_reason_name'
			},
			title: {
				type: String,
				default: ''
		...
		watch: {
			range(list) {
				// console.log(list,'sdf1111111111111111111sdf')
				this._initTree(list);
			},
			multiple() {
		...
		methods: {
			...
			_renderTreeList(list = [], rank = 0, parentId = [], parents = []) {
			  //    console.log('sdfsf:',list)
				 // console.log('---------------------------------')
				list.forEach(item => {
					console.log(item['bad_reason_name'],'sdf')
					this.treeList.push({
						bad_reason_id:item[this.idKey],
						bad_reason_name:item['bad_reason_name'],
						source: item,
						parentId, // 父级id数组
						parents, // 父级id数组
						rank, // 层级
