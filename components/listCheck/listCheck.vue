            ...
            <view class="table-row" v-if="!loadingHistoryShow">
              <u-list class="table-list-slot" @scrolltolower="pageNext">
                <u-list-item v-for="(item, idx) in historyData" :key="idx">
                  <view
                    class="list-slot-row"
                    @click="historyContentClick(item.check_clear_card_id)"
                  >
...
export default {
  ...
  methods: {
    ...
    getById(data_head_id) {
      // 修改按钮显示/隐藏
      let right = rights("check_clear_card:canEdit");
      if (right.length) {
      ...
      let url = API.listCheckById + "/" + data_head_id;
      NET.tempRequest(url, {}, "POST")
        .then((res) => {
          if (res.data.code == 200) {
            ...
            let check_clear_card_id = res.data.data.check_clear_card_id;
            let new_is_temp_save = res.data.data.is_temp_save;
            this.disabledOperation(check_clear_card_id, new_is_temp_save);
          } else if (res.data.code == 204) {
            uni.showToast({
              title: res.data.msg,
              icon: "none",
