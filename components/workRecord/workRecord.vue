            ...
            <view class="table-row" v-if="!loadingHistoryShow">
              <u-list class="table-list-slot" @scrolltolower="pageNext">
                <u-list-item v-for="(item, idx) in historyData" :key="idx">
                  <view
                    class="list-slot-row"
                    @click="historyContentClick(item.work_card_exec_id)"
                  >
...
export default {
  ...
  methods: {
      ...
      NET.tempRequest(
        ...
        .then((res) => {
          if (res.data.code == 200) {
            ...
            // 模板ID赋值
            this.work_card_exec_id = res.data.data.work_card_exec_id;
            // 工艺模板参数赋值
            this.goods_craft_name = res.data.data.goods_craft_name;
            this.goods_craft_proc_id = res.data.data.goods_craft_proc_id;
            this.work_procedure_name = res.data.data.work_procedure_name;
    ...
    saveTemp() {
      ...
      console.log(this.checkTempList);
      console.log(this.data_head_id);
      NET.tempRequest(
        API.workRecordSaveTemp,
        {
          data_head_id: this.data_head_id,
          make_line_id: this.make_line_id,
          work_position_id: this.work_position_id,
          ...
          work_card_exec_id: this.work_card_exec_id,
          exec_result: exec_result,
          work_step_stard_name: this.work_step_stard_name,
          steps: this.checkTempList,

          goods_craft_proc_id: this.goods_craft_proc_id,
          goods_craft_name: this.goods_craft_name,
          work_procedure_name: this.work_procedure_name,
        },
        "POST"
      )
        .then((res) => {
    ...
    getById(data_head_id) {
      // 修改按钮显示/隐藏
      let right = rights("work_card_exec:canEdit");
      if (right.length) {
      ...
      let url = API.workRecordById + "/" + data_head_id;
      NET.tempRequest(url, {}, "POST")
        .then((res) => {
          if (res.data.code == 200) {
            ...
            // 作业记录模板历史check_open_work_id/是否保存或暂存值赋值
            let work_card_exec_id = res.data.data.work_card_exec_id;
            let new_is_temp_save = res.data.data.is_temp_save;
            this.disabledOperation(work_card_exec_id, new_is_temp_save);
          } else if (res.data.code == 204) {
            uni.showToast({
              title: res.data.msg,
              icon: "none",
