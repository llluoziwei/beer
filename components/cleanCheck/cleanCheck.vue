    ...
    <template v-if="showHistory == '2'">
      ...
      <view class="operate">
        ...
        <view class="operate-btn" v-if="!operateDisabled"
          ...
          ></u-button
        ></view>
        <view class="operate-state">状态：{{ operateState }}</view>
      </view>
      <!-- 信息 -->
      <view class="msg">
            ...
            <view class="table-row" v-if="!loadingHistoryShow">
              <u-list class="table-list-slot" @scrolltolower="pageNext">
                <u-list-item v-for="(item, idx) in historyData" :key="idx">
                  <view
                    class="list-slot-row"
                    @click="historyContentClick(item.check_clear_line_id)"
                  >
...
export default {
  ...
  methods: {
      ...
      NET.tempRequest(
        ...
        .then((res) => {
          if (res.data.code == 200) {
            ...
            //是否显示多媒体按钮
            if (res.data.data.check_clear_line_id) {
              this.showMult = true;
            } else {
              this.showMult = false;
            }
    ...
    changeTemp() {
      this.operateDisabled = false;
      this.operateState = "修改";
    },
    // 保存按钮点击
    saveTemp() {
      this.operateState = "保存";
      // 保存中遮罩
    ...
    getById(data_head_id) {
      // 修改按钮显示/隐藏
      let right = rights("check_clear_line:canEdit");
      if (right.length) {
      ...
      let url = API.cleanCheckById + "/" + data_head_id;
      NET.tempRequest(url, {}, "POST")
        .then((res) => {
          if (res.data.code == 200) {
            ...
            let check_clear_line_id = res.data.data.check_clear_line_id;
            let new_is_temp_save = res.data.data.is_temp_save;
            this.disabledOperation(check_clear_line_id, new_is_temp_save);
          } else if (res.data.code == 204) {
            uni.showToast({
              title: res.data.msg,
              icon: "none",
