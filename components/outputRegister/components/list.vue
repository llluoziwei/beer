<template>
	...
	<view class="body">
		<view style="position: absolute;z-index: 9999;top:20px;right:0;">
			<!-- 取数 -->
			<view class="fetch">
				<u-button @click="workHistorybtn(2)" type="primary"
					style="width: 100%; height: 100%; box-shadow: 0px 2px 6px #387bfc" color="#387BFC" icon="order"
					text="报工历史记录"></u-button>
			</view>
		</view>
		<u--form labelWidth="80" labelAlign="right">
			<view style="display: flex;">
				<u-form-item label="生产线" prop="productionLine">
					...
					<u-picker @cancel="productionLineShow = false" @confirm="productionLineConfirm" keyName="name"
						closeOnClickOverlay :show="productionLineShow" :columns="[productionLineColumns]"></u-picker>
				</u-form-item>
				<u-form-item label="工位" prop="station">
		...
		<view v-if="ladingmore==true" style="height: 100%;display: flex;align-items: center;justify-content: center;">
			<u-loading-icon inactive-color="#387bfc" size="28" mode="circle" :show="ladingmore" :vertical="true"
				text="加载中"></u-loading-icon>
		</view>
		<view v-if="ladingmore==false" class="tabel">
			<view class="tabel_row">
				...
				<view class="main" v-if="list.length<=0">
					<view
						style="height: 100%;color: #CCCCCC;display: flex;align-items: center;justify-content: center;">
						暂无数据</view>
				</view>
				<view class="main" v-if="list.length>0">
							...
							<u-collapse-item v-for="(item,index) in list" :key="index" :name="index"
								...
								<view class="collapseTitle" style="padding: 10px 0 10px 33px;" slot="title">
									<u-row>
										...
										<u-col span="1">
											<view @click.stop="openDetail(1,index)" class="headBtn">报工</view>
										</u-col>
									</u-row>
								</view>
									...
									<u-row>
										<u-col span="3">
											<view class="collapseContent">
												<view class="">上工序</view>
												<view class="">本工序</view>
												<view class="">本次可报工数</view>
												<view class="">流转卡号</view>
												<view class="">产品名称</view>
											</view>
										</u-col>
										<u-col span="3">
											<view class="collapseData">
												<view class="">{{item.pre_work_procedure_name}}</view>
												<view class="">{{item.work_procedure_name}}</view>
												<view class="">{{item.not_ok_qu}}</view>
										...
										<u-col span="3">
											<view class="collapseContent">
												<view class="">上序报工数</view>
												<view class="">本序已报数</view>
												<view class="">生产线</view>
												<view class="">产品编号</view>
												<view class="">分解卡数</view>
											</view>
										</u-col>
										<u-col span="3">
											<view class="collapseData">
												<view class="">{{item.pre_qu}}</view>
												<view class="">{{item.ok_qu}}</view>
												<view class="">{{item.make_line_name}}</view>
												<view class="">{{item.goods_code}}</view>
												<view class="">{{item.work_card_qu}}</view>
											</view>
										</u-col>
									</u-row>
								</view>
	...
	export default {
		...
		methods: {
			...
			getList() {
				...
				NET.request(API.OutPutRegister, {
					currPage: this.page,
					maxPageCount: 10,
