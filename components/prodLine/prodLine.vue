  ...
  <view class="body">
    ...
    <template v-if="showHistory == '2'">
      ...
      <view class="operate">
        ...
        <view style="margin-left: 9.31rpx" @click="makeLineShow = true">
          <u--input
            placeholder="生产线选择"
            disabled
            disabledColor="#fff"
            border="surround"
            v-model="all_make_line_name"
            fontSize="11.18rpx"
            style="height: 29.82rpx; width: 74.55rpx"
          ></u--input>
        </view>
      ...
      <template v-if="!loadingShow">
        <view class="msg">
          ...
          <view class="msg-btn msg-btn-line" v-if="all_make_line_name"
            >生产线：{{ all_make_line_name }}</view
          >
          <view class="msg-btn msg-btn-line" v-else>生产线：暂无</view>
        ...
        <template>
          <view class="history-table">
            <view class="table-header">
              ...
              <view class="table-content">生产线</view>
            ...
            <view class="table-row" v-if="!loadingHistoryShow">
              <u-list class="table-list-slot" @scrolltolower="pageNext">
                <u-list-item v-for="(item, idx) in historyData" :key="idx">
                  <view
                    class="list-slot-row"
                    @click="historyContentClick(item.check_loop_line_id)"
                  >
                    <view class="table-content">{{ item.slide_no }}</view>
                    <view class="table-content">{{ item.make_line_name }}</view
    ...
    <u-picker
      ...
      keyName="check_loop_line_type_name"
    ...
    <u-picker
      ...
      keyName="make_line_name"
      @confirm="makeLineConfirm"
      @cancel="makeLineShow = false"
    ></u-picker>
...
export default {
  ...
  methods: {
    ...
    lineTypeConfirm(event) {
      ...
      this.check_loop_line_type_id = event.value[0].check_loop_line_type_id;
      this.loop_line_type = event.value[0].check_loop_line_type_name;
    },
    // 生产线选择确认
    makeLineConfirm(event) {
      this.makeLineShow = false;
      this.all_make_line_id = event.value[0].make_line_id;
      this.all_make_line_name = event.value[0].make_line_name;
      console.log(this.all_make_line_id);
    },
    ...
    getAddTemp() {
      NET.tempRequest(
        API.prodLineFindCheck,
        { make_line_id: this.all_make_line_id },
        "POST"
      )
        .then((res) => {
          if (res.data.code == 200) {
    ...
    // 生产巡线类别接口
    getLineType() {
      NET.tempRequest(API.prodLineAllType, {}, "POST")
        .then((res) => {
          console.log(res);
    ...
    // 所有生产线接口
    getAllLine() {
      NET.tempRequest(API.prodLineAllLine, {}, "POST")
        .then((res) => {
          if (res.data.code == 200) {
    ...
    saveTemp() {
      ...
      NET.tempRequest(
        API.prodLineSaveTemp,
        {
          ...
          check_loop_line_type_id: this.check_loop_line_type_id,
          make_line_id: this.all_make_line_id,
    ...
    stagingTemp() {
      ...
      NET.tempRequest(
        API.prodLineSaveTemp,
        {
          ...
          make_line_id: this.all_make_line_id,
          is_temp_save: this.is_temp_save,
          exec_result: exec_result, //检查结果(即校验结果，0通过1未通过)
          work_step_stard_name: this.work_step_stard_name,
          steps: this.checkTempList,
        },
        "POST"
    ...
    getById(data_head_id) {
      // 修改按钮显示/隐藏
      let right = rights("check_loop_line:canEdit");
      if (right.length) {
      ...
      let url = API.prodLineFindById + "/" + data_head_id;
      NET.tempRequest(url, {}, "POST")
        .then((res) => {
          if (res.data.code == 200) {
            ...
            let check_loop_line_id = res.data.data.check_loop_line_id;
            let new_is_temp_save = res.data.data.is_temp_save;
            this.disabledOperation(check_loop_line_id, new_is_temp_save);
          } else if (res.data.code == 204) {
            uni.showToast({
              title: res.data.msg,
              icon: "none",
